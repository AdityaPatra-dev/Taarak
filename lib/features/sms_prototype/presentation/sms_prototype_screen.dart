  ...
  Future<void> _sendPacket() async {
    ...
    if (fixResult case Failed<GeoTag>(:final failure)) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Could not get a location fix: ${failure.message}')));
      return;
    }
        ...
        children: [
          ...
          if (_lastEncoded != null) ...[
            ...
            OutlinedButton.icon(
              ...
              label: const Text('Simulate this packet arriving on another device'),
            ),
          ],
          const SizedBox(height: 24),
          Text('Sent (${sentMessages.length})', style: Theme.of(context).textTheme.titleSmall),
          ...
          if (receivedPackets.isEmpty)
            ...
            for (final packet in receivedPackets) _ReceivedPacketTile(packet: packet),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Card(
      child: ListTile(
        ...
        title: Text('${packet.type} from ${packet.originId} (${packet.priority.name})'),
        subtitle: Text(
