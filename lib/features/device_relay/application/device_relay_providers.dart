...
final relayActivityProvider =
    StateNotifierProvider<RelayActivityNotifier, List<DeviceRelayOutcome>>((ref) {
      final notifier = RelayActivityNotifier(ref.watch(deviceRelayServiceProvider));
      final subscription = ref.watch(relayTransportProvider).incomingBroadcasts.listen((
        raw,
      ) {
...
class RelayActivityNotifier extends StateNotifier<List<DeviceRelayOutcome>> {
  ...
  Future<void> handleIncoming(String raw, {required String thisDeviceId}) async {
    final outcome = await _service.handleIncoming(raw, thisDeviceId: thisDeviceId);
