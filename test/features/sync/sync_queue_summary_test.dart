...
void main() {
  ...
  group('syncQueueSummaryMessage', () {
    test('an empty queue reads as up to date, not as zero items pending', () {
      const summary = SyncQueueSummary();
      expect(syncQueueSummaryMessage(summary), 'Up to date');
    });
    ...
    test(
      ...
      () {
        ...
        expect(
          ...
          '3 items queued, will send when possible',
        );
      },
    );
    ...
    test('a singular pending item is grammatically correct', () {
      ...
      expect(
        syncQueueSummaryMessage(summary),
        '1 item queued, will send when possible',
      );
    });
    ...
    test('a single stalled item is grammatically correct', () {
      ...
      expect(
        syncQueueSummaryMessage(summary),
        "1 item couldn't be sent after repeated attempts",
      );
    });
  });
}
